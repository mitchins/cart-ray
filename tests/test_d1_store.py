...
def test_d1_store_persists_an_idempotent_checkout_and_outbox(fixture_catalogue):
...
def test_d1_store_enforces_leases_and_recovers_expired_leases(fixture_catalogue):
    database = MockD1Database()
    migration = Path(__file__).parents[1] / "migrations" / "0001_commerce_kernel.sql"
    database.conn.executescript(migration.read_text())
...
def test_d1_store_race_loser_rechecks_the_winner_lease(fixture_catalogue, monkeypatch):
    database = MockD1Database()
    migration = Path(__file__).parents[1] / "migrations" / "0001_commerce_kernel.sql"
    database.conn.executescript(migration.read_text())
...
def test_d1_store_rejects_a_conflicting_redirect_after_the_first_is_persisted(fixture_catalogue):
    database = MockD1Database()
    migration = Path(__file__).parents[1] / "migrations" / "0001_commerce_kernel.sql"
    database.conn.executescript(migration.read_text())
