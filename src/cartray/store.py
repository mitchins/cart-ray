...
class SqliteOrderStore:
    ...
    def start_or_load(self, order: CheckoutOrder, *, nonce: str, now: int | None = None) -> CheckoutStart:
    ...
    def attach_redirect(self, order_id: str, redirect: CheckoutRedirect, *, now: int | None = None) -> None:
        now = int(time.time()) if now is None else now
        with self.connection:
            self.connection.execute(
                """UPDATE checkout_sessions
            ...
            self._insert_outbox(
                ...
                {"order_id": order_id, "session_id": redirect.session_id},
                now,
            )
    ...
    def find_order_by_request(self, checkout_request_id: str, request_fingerprint: str) -> CheckoutOrder | None:
        row = self.connection.execute(
        ...
        if row["request_fingerprint"] != request_fingerprint:
            raise IdempotencyConflict("checkout request ID was reused with a different cart")
        return self.load_order(row["order_id"])

    def load_order(self, order_id: str) -> CheckoutOrder:
